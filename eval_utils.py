...
def get_det_logits(x, x_preds, base_detectors, sess):
    ...
    for classidx in range(10):
        ...
        det_logits[assign] = sess.run(base_detectors[classidx].logits,
                                      feed_dict=feed_dict)
    return det_logits
...
def get_adv_errors(x_adv, y, logit_ths, classifier, base_detectors, sess):
  ...
  """
    adv_preds = sess.run(classifier.predictions,
                         feed_dict={classifier.x_input: x_adv})
    det_logits = get_det_logits(x_adv, adv_preds, base_detectors, sess)
    errors = [
        np.logical_and(det_logits > th, adv_preds != y).mean()
        for th in logit_ths
    ]
    return errors
...
def get_fpr(x_adv, y, logit_ths, classifier, base_detectors, sess):
  ...
  at the same time successfully fool the base detectors (det_logits > th)"""
    return get_adv_errors(x_adv, y, logit_ths, classifier, base_detectors,
                          sess)
...
def get_nat_accs(x_nat, y, logit_ths, classifier, base_detectors, sess):
    ...
    nat_preds = sess.run(classifier.predictions,
                         feed_dict={classifier.x_input: x_nat})
...
def get_detector_ckpt(eps):
    ...
    if eps == 0.3:
        ...
        checkpoints = [
            os.path.join(
                dir,
                'ovr_class{}_{}_distance{}-{}'.format(i, 'Linf', 0.3, best[i]))
        ...
        checkpoints = [
            os.path.join(
                dir,
                'ovr_class{}_{}_distance{}-{}'.format(i, 'Linf', 0.5, best[i]))
            for i in range(num_classes)
        ]
    elif eps == 2.5:
        ...
        checkpoints = [
            os.path.join(
                dir,
                'ovr_class{}_{}_distance{}-{}'.format(i, 'L2', 2.5, best[i]))
            for i in range(num_classes)
        ]
    elif eps == 5.0:
        ...
        checkpoints = [
            os.path.join(
                dir,
                'ovr_class{}_{}_distance{}-{}'.format(i, 'L2', 5.0, best[i]))
            for i in range(num_classes)
        ]
    return checkpoints
    ...
    def __init__(self, eps):
        ...
        for i in range(self.num_classes):
            ...
            self.__base_detectors.append(
                Detector(var_scope=scope, dataset='MNIST'))
