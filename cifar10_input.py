...
class CIFAR10Data(object):
    ...
    """
    def __init__(self, path, target_class=None, select=None):
        train_filenames = ['data_batch_{}'.format(ii + 1) for ii in range(5)]
        eval_filename = 'test_batch'
        metadata_filename = 'batches.meta'

        if target_class is not None:
            assert target_class in range(10)
        if select is not None:
            assert select in ['target', 'others']
        ...
        train_mask = eval_mask = None
        if select == 'target':
            train_mask = train_labels == target_class
            eval_mask = eval_labels == target_class
        elif select == 'others':
            train_mask = train_labels != target_class
            eval_mask = eval_labels != target_class

        if train_mask is not None and eval_mask is not None:
            self.train_data = DataSubset(train_images[train_mask], train_labels[train_mask])
            self.eval_data = DataSubset(eval_images[eval_mask], eval_labels[eval_mask])
        else:
