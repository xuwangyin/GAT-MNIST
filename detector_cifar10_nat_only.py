...
test_batch_size_target = 1000
test_batch_size_others = 1000
...
if args.test_model is not None:
  ...
  test_attack = PGDAttack(classifier, None,
                          ...
                          x_min=x_min, x_max=x_max, lambda_=1, loss_fn='adv_only',
...
train_attack = PGDAttack(classifier, detector,
                         ...
                         loss_fn='one-vs-rest', distance_type=args.distance_type, target_class=args.target_class)
...
with tf.Session() as sess:
  ...
  for step in range(max_num_training_steps):
    ...
    feed_dict = {detector.x_input: x_batch_with_nat, detector.y_input: y_batch_with_nat}
    ...
    if step % num_summary_steps == 0:
      detector_logits, y_batch_pred, f_score, precision, recall, acc, balanced_acc, tpr, fpr = sess.run(
        [detector.logits, detector.predictions, detector.f_score, detector.precision, detector.recall, detector.accuracy,
         detector.balanced_accuracy, detector.true_positive_rate, detector.false_positive_rate],
        feed_dict=feed_dict)
      fpr_, tpr_, thresholds = roc_curve(y_batch_with_nat, detector_logits)
      roc_auc = auc(fpr_, tpr_)

      print('=== train auc {:.4f}, f-score {:.4f}, precision {:.4f}, recall {:.4f}, '
            'acc {:.4f}, balanced_acc {:.4f} tpr {:.4f} fpr {:.4f} '.format(
        roc_auc, f_score, precision, recall, acc, balanced_acc, tpr, fpr), end='|')
      print('pos {:.4f}, true pos {:.4f}, target {}, others {} '.format(np.sum(y_batch_pred),
                                                                       np.sum(np.bitwise_and(y_batch_pred, y_batch_with_nat)),
                                                                       np.sum(y_batch_with_nat),
                                                                       np.sum(1 - y_batch_with_nat)))
      ...
      test_detector_logits, y_pred_test, f_score, precision, recall, acc, balanced_acc, tpr, fpr = sess.run(
        [detector.logits, detector.predictions, detector.f_score, detector.precision, detector.recall,
         detector.accuracy,
         detector.balanced_accuracy, detector.true_positive_rate, detector.false_positive_rate],
        feed_dict={detector.x_input: x_test_with_nat, detector.y_input: y_test_with_nat})
      ...
      print('pos {:.4f}, true pos {:.4f}, target {}, others {} '.format(np.sum(y_pred_test),
                                                                       np.sum(
                                                                         np.bitwise_and(y_pred_test, y_test_with_nat)),
                                                                       np.sum(y_test_with_nat),
                                                                       np.sum(1 - y_test_with_nat)))
    ...
    print('step {}/{}, '
      ...
      acc, balanced_acc, tpr, fpr))

    # Write a checkpoint
    if step % num_checkpoint_steps == 0:
      model_name = 'checkpoints/{}/detector_ovr_nat_only_class{}_{}_distance{}'.format(
