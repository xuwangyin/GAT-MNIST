...
model_name = 'ovr_class{}_{}_distance{}'.format(args.target_class, args.norm, args.epsilon)

train_attack = PGDAttackDetector(detector, max_distance=args.epsilon,
                                 num_steps=args.train_steps, step_size=args.step_size,
                                 random_start=True, x_min=x_min, x_max=x_max,
                                 batch_size=batch_size, norm=args.norm,
                                 optimizer=args.train_optimizer)

test_attack = PGDAttackDetector(detector, max_distance=args.epsilon,
                                num_steps=200, step_size=args.step_size,
...
with tf.Session() as sess:
  ...
  for epoch in range(1, 101):
    ...
    for i in range(0, x_train.shape[0], batch_size*10):
      ...
      tic = time.time()
      x_batch_others_adv = train_attack.perturb(x_batch_others, None, sess)
      toc = time.time()
      ...
      fpr_, tpr_, thresholds = roc_curve(y_batch_with_adv, batch_detector_logits)
      roc_auc = auc(fpr_, tpr_)

      print('epoch {}, {}/{}'.format(epoch, i, x_train.shape[0]), end='| ')

      print('train auc {:.4f}, f-score {:.4f}, precision {:.4f}, recall {:.4f}, '.format(roc_auc, f_score, precision, recall), end='')
      print('acc {:.4f}, balanced_acc {:.4f} tpr {:.4f} fpr {:.4f},'.format(acc, balanced_acc, tpr, fpr), end='')
      print(' pos {}, neg {}'.format(x_batch_target.shape[0], x_batch_others_adv.shape[0]))
    ...
    y_test_with_adv = np.concatenate(
      [np.ones(x_val_target.shape[0], dtype=np.int64), np.zeros(x_val_others_adv.shape[0], dtype=np.int64)])
    ...
    roc_auc = auc(fpr_, tpr_)
    print('===epoch {}, test auc {:.6f}, f-score {:.4f}, precision {:.4f}, recall {:.4f}, acc {:.4f}, balanced_acc {:.4f}, tpr {:.4f} fpr {:.4f} '.format(
