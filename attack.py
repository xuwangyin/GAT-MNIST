...
from models import PGDAttackDetector, PGDAttackClassifier, PGDAttackAda
from utils import *
...
def allbar():
  ...
  files = ['detector-adv-bar.npz', 'adaptive-adv-bar.npz', 'adaptive-cw-adv-bar.npz']
  attack_names = ['detector-adv', 'adaptive-adv', 'adaptive-adv-cw']
  fpr_bars, hit_rate_bars = [], []
  bars, barnames = [], []
  for i, f in enumerate(files):
    data = np.load(f)
    fpr, tpr, hit_rates = data['fpr'], data['tpr'], data['hit_rates']
    ind = np.arange(fpr.shape[0])
    p1 = plt.bar(ind+i*width, fpr, width, color=colors[2*i+1])
    p2 = plt.bar(ind+i*width, hit_rates, width, color=colors[2*i])
    bars += [p1, p2]
    barnames += [attack_names[i] + ' FPR', attack_names[i] + ' success rate']
  
  plt.ylabel('Percentage')
  plt.xlabel('TPR')
  #plt.title('Scores by group and gender')
  plt.xticks(ind+1*width, map(str, np.round(tpr, 5)))
  #plt.yticks(np.arange(0, 81, 10))
  plt.legend(bars, barnames)
...
def bar(det_x, det_y, logits, x_with_others=None):
  hit_rates, tpr, fpr = [], [], []
  clf_y_pred = sess.run(classifier.predictions, feed_dict={classifier.x_input: det_x})
  thresholds = np.linspace(0, -20, 6)
  for th in thresholds:
    tpr_ = np.logical_and(det_y, logits >= th).sum() / det_y.sum()
    negatives = (1-det_y).sum()
    false_pos = np.logical_and((1-det_y), logits >= th)
    fpr_ = false_pos.sum() / negatives
    hit_mask = np.logical_and(false_pos, clf_y_pred==attack_class)
    hit_rate = hit_mask.sum() / negatives
    print('th {}, tpr {}, fpr {}, hit {}'.format(th, tpr_, fpr_, hit_rate))
    hit_rates.append(hit_rate)
    tpr.append(tpr_)
    fpr.append(fpr_)
    hit_samples = x[hit_mask]
    if hit_mask.sum() > 0:
      fig, axes = plt.subplots(nrows=10, ncols=10, figsize=(10, 10))
      for im, ax in zip(hit_samples, axes.ravel()):
        ax.imshow(im.reshape([28,28]), cmap='gray')
      for ax in axes.ravel():
        ax.set_axis_off()
     
      if x_with_others is not None:
        fig, axes = plt.subplots(nrows=10, ncols=10, figsize=(10, 10))
        for im, ax in zip(x_with_others[hit_mask], axes.ravel()):
          ax.imshow(im.reshape([28,28]), cmap='gray')
        for ax in axes.ravel():
          ax.set_axis_off()
      plt.show()

  fpr, tpr, hit_rates = np.array(fpr), np.array(tpr), np.array(hit_rates)
  np.savez('bar.npz', fpr=fpr, tpr=tpr, hit_rates=hit_rates)
  # mask = np.logical_and(fpr < 0.1, tpr > 0.99)
  # thresholds = thresholds[mask]
  # fpr = fpr[mask]
  # tpr = tpr[mask]
  # hit_rates = hit_rates[mask]
  ind = np.arange(fpr.shape[0])
  width = 0.2
  colors = plt.cm.Paired.colors
  colors = plt.cm.tab10.colors
  p1 = plt.bar(ind, fpr, width, color=colors[0], alpha=0.5)
  p2 = plt.bar(ind, hit_rates, width, color=colors[0], alpha=0.8)
  
  plt.ylabel('%')
  plt.title('Scores by group and gender')
  plt.xticks(ind, map(str, np.round(tpr, 5)))
  #plt.yticks(np.arange(0, 81, 10))
  plt.legend((p1[0], p2[0]), ('FPR', 'Success'))
  plt.grid(True, alpha=0.3)
  plt.savefig('bar.pdf')
  plt.show() 
...
if args.method == 'ovr':
  ...
  AUC = []
  for attack_class in range(1, 2):
    x_test_target = x_test[y_test == attack_class]
    x_test_others = x_test[y_test != attack_class][:1000]
    y_test_others = y_test[y_test != attack_class][:1000]
   
    #idx = np.load('adv_logit_idx.npy')
    #x_test_others = x_test_others[idx]
    #y_test_others = y_test_others[idx]
    ...
    with tf.Session() as sess:
      ...
      adv_logits = sess.run(detector.logits, feed_dict={detector.x_input: x_test_others_adv})
      idx = np.argsort(adv_logits)[::-1]
      np.save('adv_logit_idx.npy', idx)
